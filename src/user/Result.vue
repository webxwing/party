        ...
        <div class="button-group">
          <el-button-group>
            <el-tooltip class="item" effect="light" content="回到主页" placement="top">
              <el-button type="primary"  size="small" @click="baseFun.gotoLink({ path:'/user/main/'})">返 回</el-button>
            </el-tooltip>
          </el-button-group>
        </div>
      </div>
...
//        console.log(data);
    ...
    created(){
      ...
      this.$http.post('/webapi/getAllAssess_Deaprtment_Value',qs.stringify({'session_id':this.user.sessionID,'assess_id':  this.$route.params.assess_id,'department':this.$store.state.user.department}))
        .then((d)=>{
